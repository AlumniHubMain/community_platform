...
class PostgresDB:
    ...
    async def close(self) -> None:
        ...
        if self.connector:
            try:
                # Ensure we're not already closed
                if not asyncio.get_running_loop().is_closed():
                    await asyncio.wait_for(self.connector.close(), timeout=5.0)
            except (TimeoutError, RuntimeError) as e:
                self.logger.warning("Warning during Cloud SQL connector close: {error}", error=e)
            except Exception as e:
                self.logger.exception("Error during Cloud SQL connector close: {error}", error=e)
