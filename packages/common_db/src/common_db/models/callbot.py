...
class ORMCallbotMeeting(ObjectTable):
    """
    Meetings scheduled for the callbot to join
    """

    __tablename__ = "callbot_meetings"
    __table_args__ = (
        Index("ix_callbot_meetings_google_id", "google_id", unique=True),
        Index("ix_callbot_meetings_start_time", "start_time"),
        Index("ix_callbot_meetings_end_time", "end_time"),
        {"schema": schema},
    )
...
class ORMCallbotEnabledUser(ObjectTable):
    """
    List of users approved to invite a bot to a meeting
    """

    __tablename__ = "callbot_enabled_users"
    __table_args__ = (Index("ix_callbot_enabled_users_email", "email", unique=True), {"schema": schema})
